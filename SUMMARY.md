...
| Product surfaces | `/` landing (light Agents-Trust-skinned marketing page in the cronpay template structure: compiled Tailwind sheet + template fonts served from `public/` via Workers static assets, Roam402-themed SVG assets, hero preview = live screenshot of agents-trust.com, banner/icon regenerable from `scripts/landing-mocks/`, dynamic catalog/native data), `/playground` (Inspect-402 works for anyone; Pera pay flow in beta), `/catalog`, `/receipts` (+ .json), `/api/challenge-stats` (CORS-open aggregation), `/llms.txt`, `/.well-known/agents.json` |
